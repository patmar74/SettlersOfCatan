...
public class DevCard {
	...
	/**
	 *
	 * Overloaded method Does action for the Knight card Moves the robber to a
	 * new tile
	 * 
	 * @param myRobber
	 */
	...
	/**
	 * Overloaded do action method for the Year of Plenty Dev card Player will
	 * select two resources and gain them from the banker
	 * 
	 * @param player
	 * @param firstRequest
	 * @param secondRequest
	...
	/**
	 * Overloaded method for doing the action of the Monopoly card Gets all
	 * resource cards of a requested type from all other players and gives them
	 * to the player that used the dev card.
	 */
	public void doAction(Person player, ArrayList<Person> players, ResourceType resourceRequested) {
		...
		for (Person myPlayer : players) {
			if (myPlayer != player) { // if myPlayer has same address as player
										// then skip it
